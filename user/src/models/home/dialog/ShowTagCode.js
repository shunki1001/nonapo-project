...
const ShowTagCode = (props) => {
  ...
  // Todo: restrict url
  const tag = `<script type="text/javascript">const createIframe = () => {let modal = document.createElement("div");modal.style.position = "fixed";modal.style.bottom = "0";modal.style.right = "0";modal.style.zIndex = "9999";let obj = document.createElement("iframe");obj.setAttribute("frameBorder", "0");obj.setAttribute("scrolling", "no");obj.style.width = "350px";obj.style.height = "400px";obj.src = "https://mtg-non-apo.web.app/iframe/${domain}";modal.appendChild(obj);document.body.appendChild(modal);};setTimeout(() => {const httpIndex = location.href.indexOf("://");const domainIndex = location.href.indexOf("/", httpIndex + 3);if (domainIndex !== -1) {if (location.href.slice(httpIndex + 3, domainIndex) == "${userSite}") {createIframe();}} else if (domainIndex === -1) {if (location.href.slice(httpIndex + 3) == "${userSite}") {createIframe();}}}, 6000)</script>`;
